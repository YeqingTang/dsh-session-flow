...
//
// 数据来源：~/.dsh/sessions/<workspace>/session-<uuid>/session[.v2|.v3].jsonl.zstd
...
// dsh v0.1.5 起存档升级 v3（session.v3.jsonl.zstd）：header 行拍平为顶层字段（无 data 信封），
// 事件行信封不变；v3 为唯一活文件，必须最高优先读取（0.1.5 适配）。
//
...
export function summarizeParsed({ header, title, events }) {
  ...
  // header 字段读取兼容：v3 拍平为顶层字段；v2 及以前部分宿主版本在 data 信封下——逐字段兜底（0.1.5 适配）。
  const hf = (name, fallback = null) => {
    if (!header) return fallback
    if (header[name] !== undefined) return header[name]
    if (header.data && header.data[name] !== undefined) return header.data[name]
    return fallback
  }
  return {
    id: hf('id'),
    ...
    cwd: hf('cwd'),
    agentPreset: hf('agentPreset'),
    parentSession: hf('parentSession'),
    delegationDepth: hf('delegationDepth', 0),
...
/** 定位一个会话目录里的存档文件（优先 v3，回退 v2/v1 zstd，再回退明文）。
 * dsh v0.1.5 起 v3 存档（session.v3.jsonl.zstd）为唯一持续写入的活文件，v2/v1 冻结在
 * 迁移时刻——必须优先 v3，否则迁移老会话读到冻结数据、纯 v3 新会话直接漏掉（0.1.5 适配；
 * v0.1.3 的 v2 适配同理。存档文件名是宿主实现细节，读档永远走本函数优先级，禁止散落硬编码）。 */
