...
//
// 数据来源：~/.dsh/sessions/<workspace>/<会话目录>/session[.vN].jsonl[.zstd]
// （zstd 压缩的多帧 JSONL；compression: none 时为明文）。
// dsh 的存档文件名按**代际**递增：v0 = session.jsonl，vN（N >= 1）= session.vN.jsonl
// （见 dsh-session-format 的 generationLogFilename / parseSessionFormatLogFilename）。
// 每次格式迁移换一个新的代际文件、旧代际冻结在迁移时刻，所以读取必须取代际号**最大**
// 的那份（sessionFileOf）——硬编码版本号会让新代际会话全部 404「not found in
// archives」，按固定顺序「先新版再 legacy」回退则会读到冻结的旧数据（且不报错）。
// 行信封 {type,seq,time,data} 跨代际保持不变，故解码 / 解析与代际无关。
//
...
/** 存档文件名的 canonical 规则：`session.jsonl`（v0）或 `session.vN.jsonl`
 * （N >= 1，小写数字、无前导零），可带 `.zstd` 压缩后缀——与 dsh-session-format
 * 的 parseSessionFormatLogFilename 同口径（`.v0`／大写／前导零／临时名都不算）。 */
const SESSION_FILE_RE = /^session(?:\.v([1-9][0-9]*))?\.jsonl(\.zstd)?$/

/** 定位一个会话目录里的存档文件：取代际号**最大**的那份（v0 = 无版本段的
 * `session.jsonl`），同代际优先 zstd、再回退明文。
 *
 * dsh 的存档文件名按代际递增（session.v1/v2/v3…，见 dsh-session-format 的
 * generationLogFilename）：每次格式迁移换一个新文件，旧文件冻结在迁移时刻。
 * 所以既不能硬编码版本号（硬编码 v1 漏掉 v2 会话；补到 v2 后又漏掉 v3，
 * 表现为「not found in archives」404），也不能按固定顺序「先新版再 legacy」取
 * ——同时存在 legacy 与新版时会读到冻结的旧数据且不报错。按代际号取最大者，
 * 以后 dsh 再迁移到 v4/v5 也无需改动。 */
export function sessionFileOf(sessionDir) {
  let entries
  try { entries = readdirSync(sessionDir, { withFileTypes: true }) } catch { return null }
  let best = null
  let bestVersion = -1
  for (const entry of entries) {
    ...
    if (version > bestVersion) {
      bestVersion = version
      best = { name: entry.name, zstd }
    } else if (version === bestVersion && zstd && !best.zstd) {
      best = { name: entry.name, zstd }
    }
  }
  return best === null ? null : join(sessionDir, best.name)
}
